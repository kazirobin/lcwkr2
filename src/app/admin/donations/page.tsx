    ...
    <div className="min-h-screen bg-background text-text py-10 px-4 sm:px-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-text/10 pb-5">
          <div>
            <Link href="/admin" className="text-xs text-text/50 hover:underline flex items-center gap-1 mb-1">
              <ArrowLeft className="w-3.5 h-3.5" /> Back to Admin Console
            </Link>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              <HeartHandshake className="w-6 h-6 text-rose-500" /> Manage Donors & Contributions
            </h1>
          </div>
          <div className="text-xs font-mono bg-text/5 border border-text/10 px-4 py-2 rounded-xl">
            Total Records: <span className="font-bold text-rose-500">{donations.length}</span>
          </div>
        </div>
        ...
        <div className="p-6 rounded-3xl bg-text/5 border border-text/10 space-y-4 shadow-sm">
          <h2 className="text-base font-bold flex items-center gap-2">
            {editingId ? <Pencil className="w-4 h-4 text-amber-500" /> : <Plus className="w-4 h-4 text-emerald-500" />}
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
            <input
              ...
              className="bg-background border border-text/10 rounded-xl px-3 py-2 text-sm outline-none focus:border-rose-500"
            />
            <input
              type="text"
              required
            ...
            <input
              ...
              className="bg-background border border-text/10 rounded-xl px-3 py-2 text-sm uppercase outline-none focus:border-rose-500"
            />
            <input
              type="number"
              required
            ...
            <div className="sm:col-span-2 md:col-span-5 flex gap-2 justify-end mt-2">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="px-5 py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-sm disabled:opacity-50"
              >
        ...
        <div className="bg-text/5 border border-text/10 rounded-3xl overflow-hidden shadow-sm">
          {loading ? (
            ...
            <div className="p-8 text-center text-sm text-text/50">No donor records available.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-text/5">
                  {donations.map((donor) => (
                    <tr key={donor._id || donor.trxId} className="hover:bg-text/[0.02] transition">
                      ...
                      <td className="p-4 text-xs font-mono font-bold text-rose-600">
                        <span className="flex items-center gap-1">
                          <CreditCard className="w-3.5 h-3.5 text-rose-400" />
                          {donor.trxId}
                        </span>
                      </td>
                      <td className="p-4 text-xs font-bold text-emerald-600">৳{donor.amount}</td>

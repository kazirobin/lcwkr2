...
export default function StudentDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-background text-text flex flex-col items-center justify-center p-4 text-center space-y-4 transition-colors duration-300">
        <h2 className="text-lg sm:text-xl font-bold text-secondary">
          Student &quot;{rollNumber}&quot; Not Found
        </h2>
        <Link
          href="/academy/students"
          className="text-xs sm:text-sm text-text/60 hover:text-text underline transition-colors"
        >
  ...
  const getCourseAttendanceStats = (courseId: string) => {
    ...
    const rate =
      totalHeld > 0
        ? `${((attendedCount / totalHeld) * 100).toFixed(1)}%`
        : "100%";
  ...
  const studentExams = academyData.courses.flatMap((c) =>
    (c.weekendExams ?? []).flatMap((e) => {
      const res = (e.results ?? []).find(
        (r) => String(r.rollNumber).trim() === targetRoll,
      );
      if (!res) return [];
      return [
        {
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2 text-xs text-text/70">
                ...
                {isAdminUnlocked ? (
                  ...
                  >
                    ...
                    <span className="text-[10px] bg-primary/20 text-primary px-1.5 py-0.2 rounded font-sans">
                      Chat
                    </span>
                  </a>
                ) : (
                  ...
                  >
                    ...
                    <span className="font-mono">
                      {maskPhoneNumber(student.whatsapp)}
                    </span>
                    <span className="text-[10px] text-text/30 group-hover:text-text/40 font-sans">
                      (Hidden)
                    </span>
                  </button>
                )}
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            {student.enrolledCourseIds?.map((cId) => {
              ...
              const { totalHeld, attendedCount, absentCount, rate, classes } =
                getCourseAttendanceStats(cId);
              const examAvg = getCourseExamAverage(cId);

              return (
                <div
                  key={cId}
                  className="p-4 sm:p-6 rounded-2xl bg-text/5 border border-text/10 space-y-4 sm:space-y-5 flex flex-col justify-between transition-colors hover:border-primary/30"
                >
                  <div className="space-y-3 sm:space-y-4">
                    {/* Course Header */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div>
                        ...
                        <h3 className="font-bold text-text text-sm sm:text-base mt-1">
                          {crs?.courseName || cId}
                        </h3>
                      </div>
                      <Link
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-2.5 text-center">
                      <div className="p-2 sm:p-3 bg-text/5 rounded-xl border border-text/10">
                        <span className="text-text/50 block text-[9px] sm:text-[10px] uppercase font-semibold">
                          Total Classes
                        </span>
                        <span className="font-mono font-bold text-text text-sm sm:text-base mt-0.5 block">
                          {totalHeld}
                        </span>
                      </div>
                      <div className="p-2 sm:p-3 bg-text/5 rounded-xl border border-text/10">
                        <span className="text-primary block text-[9px] sm:text-[10px] uppercase font-semibold">
                          Attended
                        </span>
                        <span className="font-mono font-bold text-primary text-sm sm:text-base mt-0.5 block">
                          {attendedCount}
                        </span>
                      </div>
                      <div className="p-2 sm:p-3 bg-text/5 rounded-xl border border-text/10">
                        <span className="text-secondary block text-[9px] sm:text-[10px] uppercase font-semibold">
                          Absent
                        </span>
                        <span className="font-mono font-bold text-secondary text-sm sm:text-base mt-0.5 block">
                          {absentCount}
                        </span>
                      </div>
                      <div className="p-2 sm:p-3 bg-text/5 rounded-xl border border-text/10">
                    ...
                    <div className="space-y-2 pt-2 border-t border-text/10">
                      <span className="text-[11px] sm:text-xs font-semibold text-text/50 block">
                      ...
                      {totalHeld === 0 ? (
                        ...
                        <div className="space-y-1.5">
                          {classes.map((cls) => {
                            const isPresent = cls.presentStudents?.some(
                              (r) => String(r).trim() === targetRoll,
                            );
                            return (
                              <div
                                key={cls.classId}
                              ...
                              >
                                <div className="space-y-0.5 min-w-0">
                                  <span className="font-mono text-text/40 text-[10px] sm:text-[11px] block truncate">
                                    {cls.classId} • {cls.date}
                                  </span>
                  ...
                  <div className="p-2.5 sm:p-3 bg-text/5 rounded-xl border border-text/10 flex justify-between items-center text-xs">
                    <span className="text-text/50 text-[11px] sm:text-xs">
                      Dynamic Exam Average:
                    </span>
        ...
        <div className="p-4 sm:p-6 rounded-2xl bg-text/5 border border-text/10 space-y-3 sm:space-y-4 transition-colors">
          <h2 className="text-base sm:text-lg font-bold text-text flex items-center gap-2">
            ...
            Assessment History & Remarks
          </h2>

          {studentExams.length === 0 ? (
            <p className="text-xs sm:text-sm text-text/40">
              No examination scores recorded.
            </p>
          ) : (
                ...
                <tbody className="divide-y divide-text/10">
                  {studentExams.map((ex, idx) => (
                    <tr key={idx} className="hover:bg-text/5 transition-colors">
                      <td className="p-2.5 sm:p-3 font-mono text-text/50 whitespace-nowrap">
                        {ex.date}
                      </td>
                      <td className="p-2.5 sm:p-3 font-semibold text-text">
                        {ex.examTitle}
                      </td>
                      <td className="p-2.5 sm:p-3 font-mono text-text/50">
                        {ex.courseId}
                      </td>
                      <td className="p-2.5 sm:p-3 font-mono font-bold text-text whitespace-nowrap">
                      ...
                      <td className="p-2.5 sm:p-3 text-text/40 italic">
                        {ex.remarks || "—"}
                      </td>
                    </tr>
                  ))}
          ...
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="w-full max-w-sm bg-background border border-text/10 rounded-2xl p-5 sm:p-6 shadow-2xl space-y-4 transition-colors">
              ...
              <p className="text-xs text-text/50">
                Enter your instructor/admin passkey to reveal full contact
                details and direct chat actions.
              </p>
